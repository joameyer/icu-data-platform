#!/usr/bin/env bash
#SBATCH --job-name=asic-v3-static-registry-completion
#SBATCH --output=/hpcwork/jrc_combine/joana/icu_data_platform_v3/asic/runs/production/static-registry-completion-%j.out
#SBATCH --error=/hpcwork/jrc_combine/joana/icu_data_platform_v3/asic/runs/production/static-registry-completion-%j.err
...
JOB_SCRIPT=${PROJECT_ROOT}/asic/slurm/run_static_registry_completion_audit_production.sh
...
if [[ ! "${REGISTRY_REVIEW_RUN_ID}" =~ ^[A-Za-z0-9][A-Za-z0-9_-]{0,79}$ ]]; then
    echo "error: invalid immutable registry-review run ID" >&2
    exit 1
fi
...
for required_path in \
    ...
    if [[ ! -f "${required_path}" ]]; then
        echo "error: immutable static registry-completion input evidence is incomplete" >&2
        exit 1
    fi
done
mkdir -p "${RUNS_ROOT}"
...
if [[ "${ASIC_STATIC_REGISTRY_COMPLETION_WORKER:-0}" != "1" ]]; then
    ...
    exec sbatch \
        --export=ALL,REGISTRY_REVIEW_RUN_ID="${REGISTRY_REVIEW_RUN_ID}",ASIC_STATIC_REGISTRY_COMPLETION_WORKER=1 \
        "${JOB_SCRIPT}"
fi

if [[ -z "${SLURM_JOB_ID:-}" ]]; then
    echo "error: static registry-completion worker must run inside a Slurm allocation" >&2
    exit 1
fi
...
if [[ ${audit_exit} -eq 2 ]]; then
    echo "static_registry_completion_audit_completed_with_human_review_findings=true"
    exit 0
fi
exit "${audit_exit}"
