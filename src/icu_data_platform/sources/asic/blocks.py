...
BLOCK_COUNT_DISTRIBUTION_COLUMNS = [
    ...
    "completed_block_count",
    "stay_count",
    "hospital_retained_stay_count",
    "proportion_of_hospital_retained_stays",
]
...
def _prepare_retained_stays(chapter1_cohort_df: pd.DataFrame) -> pd.DataFrame:
    _require_columns(
        chapter1_cohort_df,
    ...
    if retained["stay_id_global"].duplicated().any():
        ...
        raise ValueError(
            "Retained ASIC Chapter 1 stays must be unique before 8-hour block construction. "
            f"Duplicate stay_id_global values found: {duplicate_ids}"
        )

    admission_numeric = pd.to_numeric(retained["icu_admission_time"], errors="coerce")
    invalid_anchor = ~admission_numeric.eq(0).fillna(False)
    if invalid_anchor.any():
        invalid_stays = (
            retained.loc[invalid_anchor, "stay_id_global"].dropna().astype("string").tolist()[:10]
        )
        raise ValueError(
    ...
    proxy_timedelta = pd.to_timedelta(retained["icu_end_time_proxy"], errors="coerce")
    retained["icu_end_time_proxy_hours"] = proxy_timedelta.dt.total_seconds() / 3600.0
    retained["completed_block_count"] = pd.Series(0, index=retained.index, dtype="Int64")

    non_negative_proxy = (
        retained["icu_end_time_proxy_hours"].notna() & retained["icu_end_time_proxy_hours"].ge(0)
    )
    retained.loc[non_negative_proxy, "completed_block_count"] = (
        retained.loc[non_negative_proxy, "icu_end_time_proxy_hours"] // BLOCK_SIZE_HOURS
    ).astype("Int64")
    retained["has_completed_block"] = retained["completed_block_count"].ge(1)
    retained["ends_exactly_on_8h_boundary"] = (
        non_negative_proxy
        & retained["icu_end_time_proxy_hours"].mod(BLOCK_SIZE_HOURS).eq(0)
    )
    retained["terminal_block_end_h"] = retained["completed_block_count"] * BLOCK_SIZE_HOURS

    ordered_columns = STAY_BLOCK_COUNT_COLUMNS + [
        column for column in retained.columns if column not in STAY_BLOCK_COUNT_COLUMNS
    ]
    return retained[ordered_columns].sort_values(
        ["hospital_id", "stay_id_global"]
    ).reset_index(drop=True)


def _retained_dynamic_input(
...
def _build_block_count_distribution_by_hospital(stay_block_counts: pd.DataFrame) -> pd.DataFrame:
    ...
    hospital_totals = (
        ...
        .size()
        .rename("hospital_retained_stay_count")
        .reset_index()
    )
    distribution = (
    ...
    distribution["proportion_of_hospital_retained_stays"] = (
        distribution["stay_count"] / distribution["hospital_retained_stay_count"]
    )
...
def _build_qc_summary(
    ...
    return pd.DataFrame(
        [
            ...
            {
                "metric": "retained_chapter1_stays_total",
                "value": int(stay_block_counts.shape[0]),
                "note": "Retained ASIC Chapter 1 stays entering 8-hour block construction.",
            },
            {
                "metric": "retained_dynamic_rows_total",
                "value": int(retained_dynamic.shape[0]),
                "note": "Harmonized dynamic rows after Chapter 1 site-level and stay-level exclusions.",
            },
            {
                "metric": "retained_stays_with_zero_blocks",
                "value": zero_block_count,
                "note": (
                    "Retained stays with no fully completed 8-hour block because "
                    "icu_end_time_proxy < 8h or is unusable."
                ),
            },
            {
                "metric": "retained_stays_with_one_or_more_blocks",
                "value": one_plus_block_count,
                "note": "Retained stays with at least one fully completed 8-hour block.",
            },
            {
                "metric": "block_rows_total",
                "value": int(block_index.shape[0]),
                "note": "Block-level index rows emitted for retained ASIC Chapter 1 stays.",
            },
            {
                "metric": "negative_dynamic_time_rows_in_retained_input",
                "value": negative_dynamic_time_rows,
                "note": "Invalid retained-input dynamic rows with time < 0 hours.",
            },
            {
                "metric": "negative_dynamic_time_stays_in_retained_input",
                "value": int(negative_dynamic_time_qc.shape[0]),
                "note": "Retained stays containing one or more invalid negative dynamic times.",
            },
            {
            ...
            {
                "metric": "retained_stays_ending_exactly_on_8h_boundary",
                "value": int(exact_boundary_stays.shape[0]),
                "note": "Retained stays whose icu_end_time_proxy lands exactly on an 8-hour boundary.",
            },
            {
            ...
            {
                "metric": "block_index_is_pre_prediction_instance_eligibility_only",
                "value": 1,
                "note": (
                    "Block structure is preserved separately from later valid prediction-instance "
                    "eligibility checks."
                ),
            },
        ]
...
def build_asic_chapter1_8h_blocks(
    chapter1_cohort_df: pd.DataFrame,
    dynamic_df: pd.DataFrame,
) -> ASICChapter1BlockResult:
    stay_block_counts = _prepare_retained_stays(chapter1_cohort_df)
